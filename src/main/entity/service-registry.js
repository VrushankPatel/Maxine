...
class ServiceRegistry extends EventEmitter {
  ...
  register(serviceName, nodeInfo) {
    if (config.lightningMode) {
      ...
      const weight =
        nodeInfo.metadata && nodeInfo.metadata.weight ? parseInt(nodeInfo.metadata.weight) : 1;
      const version =
        nodeInfo.metadata && nodeInfo.metadata.version ? nodeInfo.metadata.version : null;
      ...
      if (!this.registry.has(fullServiceName)) {
        this.registry.set(fullServiceName, {
          nodes: new Map(),
          healthyNodes: [],
          roundRobinIndex: 0,
        });
      ...
      if (global.broadcast)
        global.broadcast('service_registered', { serviceName: fullServiceName, nodeId: nodeName });
      return nodeName;
    } else {
      ...
      this.publishChange({
        ...
        serviceName,
        nodeName,
        data: { node: service.nodes[nodeName] },
      });
      this.debounceSave();
      return nodeName;
    }
  ...
  ultraFastGetRandomNode(serviceName, strategy = 'round-robin', clientId = null) {
    if (config.lightningMode) {
      ...
      const availableNodes =
        ...
        serviceData.array.filter(
          (node) =>
            !this.isInMaintenance(serviceName, node.nodeName) &&
            !this.isInDraining(serviceName, node.nodeName)
        );
  ...
  getCanaryNode(serviceName) {
    ...
    const canaryNodes = service.healthyNodes.filter((node) =>
      config.canaryNodes.includes(node.nodeName)
    );
  ...
  updateAvailable(serviceName) {
    if (config.lightningMode && this.ultraFastHealthyNodes.has(serviceName)) {
      ...
      const serviceData = this.ultraFastHealthyNodes.get(serviceName);
      if (serviceData) {
        serviceData.available = serviceData.array.filter(
          (node) =>
            !this.isInMaintenance(serviceName, node.nodeName) &&
            !this.isInDraining(serviceName, node.nodeName)
        );
  ...
  async publishChange(change) {
    if (config.redisEnabled) {
      ...
      if (this.redisPublisher) {
        ...
        this.redisPublisher
          .publish('maxine-registry-changes', JSON.stringify(change))
          .catch((err) => {
  ...
  analyzeServiceScaling = (serviceName, service) => {
    ...
    } else if (healthyCount === 0 && totalNodes > 0) {
      // All nodes unhealthy
      action = 'scale_up';
      reason = 'All nodes unhealthy - need more instances';
      confidence = 0.95;
    } else if (healthyCount < totalNodes * 0.5) {
    ...
    if (action !== 'none') {
      return {
        ...
        recommendedInstances:
          action === 'scale_up'
            ? Math.max(1, Math.ceil(totalNodes * 1.5))
            : action === 'scale_down'
              ? Math.max(1, Math.floor(totalNodes * 0.8))
              : totalNodes,
      };
    }
  ...
  addChange = (type, serviceName, nodeName, data) => {
    ...
    if (config.natsEnabled) {
      ...
      if (this.natsClient) {
        this.natsClient
          .publish(config.natsSubject, JSON.stringify(change))
          .catch((err) => consoleError('NATS send error:', err));
      }
    }
    // Send to MQTT
  ...
  getAliasesForService = (serviceName) => {
    return this.serviceAliasesReverse.has(serviceName)
      ? Array.from(this.serviceAliasesReverse.get(serviceName))
      : [];
  };
  ...
  getBlacklistedNodes = (serviceName) => {
    return this.blacklistNodes.get(serviceName)
      ? Array.from(this.blacklistNodes.get(serviceName))
      : [];
  };
  ...
  getServiceDependencies = (serviceName) => {
    return this.serviceDependencies.has(serviceName)
      ? Array.from(this.serviceDependencies.get(serviceName))
      : [];
  };
  ...
  backup = () => {
    return {
      ...
      groupIndex: Object.fromEntries(
        Array.from(this.groupIndex.entries()).map(([k, v]) => [
          k,
          Object.fromEntries(
      ...
      ultraFastHealthyNodes: Object.fromEntries(
        Array.from(this.ultraFastHealthyNodes.entries()).map(([k, v]) => [
          k,
          config.lightningMode ? v : v.array,
        ])
      ),
      lightningNodes: config.lightningMode
        ? Object.fromEntries(
            Array.from(this.registry.entries()).map(([k, v]) => [
              k,
              { ...v, nodes: Array.from(v.nodes.entries()) },
            ])
          )
        : {},
  ...
  restore = (data) => {
    ...
    this.webhooks = new Map(Object.entries(data.webhooks || {}).map(([k, v]) => [k, new Set(v)]));
    this.tagIndex = new Map(Object.entries(data.tagIndex || {}).map(([k, v]) => [k, new Set(v)]));
    this.groupIndex = new Map(
      Object.entries(data.groupIndex || {}).map(([k, v]) => [
        k,
        new Map(Object.entries(v).map(([k2, v2]) => [k2, new Map(Object.entries(v2))])),
      ])
    );
    ...
    if (config.lightningMode) {
      ...
      this.registry = new Map(
        Object.entries(data.lightningNodes || {}).map(([k, v]) => [
          k,
          { ...v, nodes: new Map(v.nodes) },
        ])
      );
    } else {
      this.ultraFastHealthyNodes = new Map(
  ...
  setMaintenanceMode = (serviceName, nodeName, inMaintenance) => {
    ...
    if (inMaintenance) {
      ...
      if (this.maintenanceNodes.has(serviceName)) {
        ...
        if (
          this.healthyNodeSets.has(serviceName) &&
          this.healthyNodeSets.get(serviceName).has(nodeName) &&
          !this.isInDraining(serviceName, nodeName)
        ) {
          if (!this.availableNodes.has(serviceName)) {
  ...
  isInMaintenance = (serviceName, nodeName) => {
    return (
      this.maintenanceNodes.has(serviceName) && this.maintenanceNodes.get(serviceName).has(nodeName)
    );
  };
    ...
    if (draining) {
      ...
      if (this.drainingNodes.has(serviceName)) {
        ...
        if (
          this.healthyNodeSets.has(serviceName) &&
          this.healthyNodeSets.get(serviceName).has(nodeName) &&
          !this.isInMaintenance(serviceName, nodeName)
        ) {
          if (!this.availableNodes.has(serviceName)) {
    ...
    if (!this.sortedHealthyCache.has(cacheKey)) {
      ...
      if (tags && tags.length > 0) {
        ...
        for (const nodeName of intersection) {
          ...
          if (
            node &&
            (!deployment || node.metadata.deployment === deployment) &&
            !this.isBlacklisted(serviceName, nodeName)
          ) {
            filtered.push(node);
          }
        }
        ...
        for (const [nodeName, node] of candidates) {
          if (node.metadata.deployment !== deployment || this.isBlacklisted(serviceName, nodeName))
            continue;
          filtered.push(node);
        }
      } else {
        filtered = Array.from(candidates.values()).filter(
          (node) => !this.isBlacklisted(serviceName, node.nodeName)
        );
      }
      if (filter) {
      ...
      if (
        filtered.length > 1 &&
        (filtered[0].metadata.priority || 0) !==
          (filtered[filtered.length - 1].metadata.priority || 0)
      ) {
  ...
  getNodeValue = (node, key) => {
    ...
    switch (key) {
      ...
      case 'health_score':
        ...
        const failures =
          this.circuitBreakerFailures.get(`${node.serviceName}:${node.nodeName}`) || 0;
  ...
  addToHealthyNodes = (serviceName, nodeName) => {
    ...
    if (
      config.maxInstancesPerService > 0 &&
      service.nodes &&
      Object.keys(service.nodes).length >= config.maxInstancesPerService
    ) {
      consoleLog(
        `Service ${serviceName} has reached maximum instances limit (${config.maxInstancesPerService})`
      );
      return;
    }
    ...
    if (config.ultraFastMode || config.lightningMode) {
      if (config.lightningMode) {
        ...
        if (!serviceData.nodes.has(nodeName)) {
          ...
          serviceData.available = serviceData.array.filter(
            (n) =>
              !this.isInMaintenance(serviceName, n.nodeName) &&
              !this.isInDraining(serviceName, n.nodeName)
          );
        }
      }
  ...
  calculateHealthScore = (serviceName, nodeName) => {
    ...
    const node = service.nodes[nodeName];
    const failureRate =
      (node.failureCount || 0) / ((node.failureCount || 0) + (node.successCount || 1)); // better calculation
    const avgResponseTime = this.getAverageResponseTime(serviceName, nodeName) || 1; // avoid division by zero
    const uptime =
      this.getHealthHistory(serviceName, nodeName).filter((h) => h.status).length /
      Math.max(1, this.getHealthHistory(serviceName, nodeName).length);
  ...
  saveToMySQL = async () => {
    ...
    try {
      ...
      await this.mysqlClient.execute(
        'INSERT INTO registry (id, data) VALUES (1, ?) ON DUPLICATE KEY UPDATE data = ?',
  ...
  saveToCassandra = async () => {
    ...
    try {
      ...
      await this.cassandraClient.execute(
        `INSERT INTO ${config.cassandraKeyspace}.registry (id, data) VALUES (?, ?)`,
        ['registry', JSON.stringify(data)]
      );
    } catch (err) {
  ...
  debounceSave = () => {
    if (
      config.extremeFastMode ||
      config.ultraFastMode ||
      (!config.persistenceEnabled &&
        !config.mongoEnabled &&
        !config.postgresEnabled &&
        !config.mysqlEnabled &&
        !config.cassandraEnabled)
    )
      return;
    if (this.saveTimeout) {
      ...
      if (dataStr) {
        ...
        for (const serviceName of data.hashRegistry || []) {
          ...
          for (const nodeName of Object.keys(nodes || {})) {
            this.addNodeToHashRegistry(serviceName, nodeName);
            if (nodes[nodeName].healthy !== false) {
              // assuming healthy is true by default
              this.addToHealthyNodes(serviceName, nodeName);
              this.addToHashRegistry(serviceName, nodeName);
            }
            this.addToTagIndex(nodeName, nodes[nodeName].metadata.tags);
  ...
  async initCassandra() {
    ...
    await this.cassandraClient.execute(
      `CREATE KEYSPACE IF NOT EXISTS ${config.cassandraKeyspace} WITH REPLICATION = {'class': 'SimpleStrategy', 'replication_factor': 1}`
    );
    await this.cassandraClient.execute(
      `CREATE TABLE IF NOT EXISTS ${config.cassandraKeyspace}.registry (id text PRIMARY KEY, data text)`
    );
  ...
  loadFromCassandra = async () => {
    ...
    try {
      const result = await this.cassandraClient.execute(
        `SELECT data FROM ${config.cassandraKeyspace}.registry WHERE id = 'registry'`
      );
    ...
    try {
      ...
      if (fs.existsSync(filePath)) {
        ...
        if (content) {
          ...
          if (config.lightningMode) {
            this.registry = new Map(
              Object.entries(data.registry || {}).map(([k, v]) => [
                k,
                { ...v, nodes: new Map(v.nodes) },
              ])
            );
          ...
          // Load health history
          this.healthHistory = new Map(
            Object.entries(data.healthHistory || {}).map(([k, v]) => [
              k,
          ...
          // Load available nodes
          this.availableNodes = new Map(
            Object.entries(data.availableNodes || {}).map(([k, v]) => [
              k,
              new Map(Object.entries(v)),
            ])
          );
  ...
  getCircuitBreaker(serviceName, nodeName) {
    const key = `${serviceName}:${nodeName}`;
    return (
      this.circuitBreaker.get(key) || { state: 'closed', failures: 0, lastFailure: 0, nextTry: 0 }
    );
  }
    ...
    for (const [name, reg] of this.federatedRegistries) {
      try {
        const response = await axios.get(
          `${reg.url}/discover?serviceName=${serviceName}&strategy=${strategy}&clientId=${clientId || ''}`,
          { timeout: 1000 }
        );
