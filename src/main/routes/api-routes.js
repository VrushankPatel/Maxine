...
} = require('../controller/maxine/envoy-controller');
...
const {
  ...
  deleteConfig,
  watchConfig,
} = require('../controller/config-control/config-controller');
...
const {
  ...
  getDependencyGraph,
  detectCycles,
} = require('../controller/dependency-controller');
...
} = require('../controller/log-control/logs-controller');
const {
  configuratorController,
  configurationController,
} = require('../controller/config-control/configurator-controller');
...
const {
  requireRole,
  requirePermission,
} = require('../controller/security/authorization-controller');
const { PERMISSIONS } = require('../security/rbac');
const {
  getRolesController,
  getUserRolesController,
  setUserRoleController,
} = require('../controller/security/role-controller');
const {
  generateApiKey,
  revokeApiKey,
  listApiKeys,
  validateApiKey,
} = require('../controller/security/api-key-controller');
...
} = require('../controller/maxine/service-mesh-ai-controller');
const {
  getServiceCallAnalytics,
} = require('../controller/maxine/service-call-analytics-controller');
const {
  getMultiCloudScalingRecommendations,
  executeMultiCloudScaling,
  getCloudProviderStatus,
} = require('../controller/maxine/multi-cloud-autoscaling-controller');
const {
  getEBPFMetrics,
  getEBPFTraces,
  ...
  detachEBPFProbe,
  getEBPFStatus,
} = require('../controller/maxine/ebpf-controller');
const {
...
} = require('../controller/maxine/distributed-persistence-controller');
const {
  getSpireStatus,
...
const discoveryLimiter =
  isHighPerformanceMode || config.ultraFastMode || !config.rateLimitEnabled
    ? null
    : createRedisRateLimiter({
...
maxineApiRoutes = maxineApiRoutes.from('maxine').from('serviceops');
...
maxineApiRoutes = maxineApiRoutes
  .post(
    'register',
    config.ultraFastMode || isLightningMode ? null : authenticationController,
    bodyParser.json(),
    registryController
  )
  ...
  .post(
    'config/set',
    config.ultraFastMode || isLightningMode ? null : authenticationController,
    config.ultraFastMode || isLightningMode ? null : limiter,
    bodyParser.json(),
    setConfig
  )
  .get(
...
maxineApiRoutes = maxineApiRoutes.delete(
  'deregister',
  config.ultraFastMode || isLightningMode ? null : authenticationController,
  limiter,
  bodyParser.json(),
...
maxineApiRoutes = maxineApiRoutes.get(
  'discover',
  config.ultraFastMode || isLightningMode ? null : authenticationController,
  discoveryLimiter,
  discoveryController
...
if (!config.ultraFastMode && !isLightningMode) {
  maxineApiRoutes = maxineApiRoutes
    .post(
      'discover/batch',
      config.ultraFastMode ? null : authenticationController,
      discoveryLimiter,
      bodyParser.json(),
      batchDiscoveryController
    )
    .get(
      'discover/info',
      config.ultraFastMode ? null : authenticationController,
    ...
    .post(
      ...
      limiter,
      bodyParser.json(),
      (req, res) => injectNetworkPartition(req, res)
    )
    .post(
      'chaos/experiment',
      authenticationController,
      ...
      limiter,
      bodyParser.json(),
      (req, res) => runChaosExperiment(req, res)
    )
    .get('chaos/experiments', authenticationController, requireRole('admin'), limiter, (req, res) =>
      getChaosExperiments(req, res)
    )
    .delete(
      ...
      requireRole('admin'),
      limiter,
      (req, res) => stopChaosExperiment(req, res)
    )
    .post(
      'chaos/reset',
      authenticationController,
      ...
      limiter,
      bodyParser.json(),
      (req, res) => resetChaos(req, res)
    )
    .get('chaos/status', authenticationController, requireRole('admin'), limiter, (req, res) =>
      getChaosStatus(req, res)
    )
    ...
    .get(
      'persistence/service-data/:serviceName/:type',
      authenticationController,
      limiter,
      loadServiceData
    )
    ...
    .post(
      ...
      limiter,
      bodyParser.json(),
      federationController.addFederatedRegistry
    )
    .post(
      'federation/remove',
      authenticationController,
      ...
      limiter,
      bodyParser.json(),
      federationController.removeFederatedRegistry
    )
    .get(
      'federation',
      authenticationController,
      limiter,
      federationController.getFederatedRegistries
    )
    .get(
      'federation/status',
      authenticationController,
      limiter,
      federationController.getFailoverStatus
    )
    ...
    .get(
      'intention/:source/:destination',
      authenticationController,
      limiter,
      getIntentionController
    )
    .post(
      'blacklist/service/add',
      authenticationController,
      requireRole('admin'),
      limiter,
      bodyParser.json(),
...
maxineApiRoutes = maxineApiRoutes
  ...
  .put(
    'config',
    config.isTestMode ? null : authenticationController,
    config.isTestMode ? null : requireRole('admin'),
    bodyParser.json(),
    configuratorController
  )
...
maxineApiRoutes = maxineApiRoutes
  ...
  .get('metrics', (req, res) =>
    res.status(200).json({ memory: process.memoryUsage(), uptime: process.uptime() })
  )
  .stepToRoot()
  .from('auth')
