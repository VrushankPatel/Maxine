...
const { validate, serviceRegistrationSchema } = require('../util/validation');

const lightningRegistryController = [
  validate(serviceRegistrationSchema),
  (req, res) => {
    const { serviceName, host, hostName, port, metadata, namespace, datacenter, tags, version, environment } =
      req.body;
    const nodeId = getServiceRegistry().register(
      serviceName,
      { host: host || hostName, port, metadata, tags, version, environment },
      namespace,
      datacenter
    );
...
const { heartbeatSchema } = require('../util/validation');

const lightningHeartbeatController = [
...
const lightningDeregisterController = [
  validate(deregisterSchema),
  (req, res) => {
    const { nodeId } = req.body;
    const parts = nodeId.split(':');
    const serviceName = parts[0];
    const nodeName = parts.slice(1).join(':');
    getServiceRegistry().deregister(serviceName, nodeName);
...
router.get('/health', (req, res) => {
  ...
  const totalNodes = Array.from(getServiceRegistry().services.values()).reduce(
    (sum, s) => sum + s.healthyNodes.size,
    0
  );
  const totalActiveConnections = Array.from(getServiceRegistry().activeConnections.values()).reduce(
    (sum, c) => sum + c,
    0
  );
  ...
  const averageResponseTime =
    responseTimes.length > 0
      ...
      : 0;
  res.end(
    `{"status":"ok","services":${services},"nodes":${totalNodes},"activeConnections":${totalActiveConnections},"averageResponseTime":${averageResponseTime.toFixed(2)}}`
  );
});
...
if (config.metricsEnabled) {
  ...
  router.get('/metrics', (req, res) => {
    ...
    const totalActiveConnections = Array.from(
      getServiceRegistry().activeConnections.values()
    ...
    const averageResponseTime =
      responseTimes.length > 0
        ...
        : 0;
    res.end(
      `{"uptime":${uptime},"requests":${requestCount},"errors":${errorCount},"services":${services},"nodes":${totalNodes},"activeConnections":${totalActiveConnections},"averageResponseTime":${averageResponseTime.toFixed(2)}}`
    );
  });
