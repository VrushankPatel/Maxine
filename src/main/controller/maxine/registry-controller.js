...
const autoscalingController = (req, res) => {
  ...
  webhooks.forEach((url) => {
    ...
    axios
      .post(url, { serviceName, action, reason, timestamp: new Date().toISOString() })
      .catch((err) => {
...
const registryController = (req, res) => {
  ...
  if (nodeCount >= config.maxInstancesPerService) {
    res
      ...
      .json({
        message: `Maximum instances per service (${config.maxInstancesPerService}) exceeded`,
      });
    return;
  }
  ...
  if (!serviceResponse) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: statusAndMsgs.MSG_INVALID_SERVICE_DATA });
    return;
  }
  ...
  audit(
    `REGISTER: service ${serviceResponse.serviceName} node ${serviceResponse.nodeName} at ${serviceResponse.address}`
  );
...
const renewLeaseController = (req, res) => {
  ...
  if (!serviceName || !nodeName) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Missing serviceName or nodeName' });
    return;
  }
  const fullServiceName = `${namespace}:${serviceName}`;
...
const heartbeatController = (req, res) => {
  ...
  if (config.lightningMode) {
    nodesToRenew = Array.from(service.nodes.values())
      .filter((n) => n.nodeName === nodeName || n.parentNode === nodeName)
      .map((n) => n.nodeName);
  } else {
    nodesToRenew = Object.keys(service.nodes).filter(
      (n) => service.nodes[n].nodeName === nodeName || service.nodes[n].parentNode === nodeName
    );
  }
...
const deregisterController = (req, res) => {
  ...
  const success = registryService.deregisterService(
    serviceName,
    nodeName,
    namespace,
    undefined,
...
const bulkHealthController = async (req, res) => {
  ...
  if (!serviceNames || !Array.isArray(serviceNames)) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Missing or invalid serviceNames array' });
    return;
  }
  const results = {};
  for (const serviceName of serviceNames) {
    const fullServiceName = version
      ? `${namespace}:${serviceName}:${version}`
...
const filteredDiscoveryController = (req, res) => {
  ...
  if (!serviceName) {
    ...
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: statusAndMsgs.MSG_DISCOVER_MISSING_DATA });
    return;
  }

  const fullServiceName = `${namespace}:${serviceName}`;
  const nodes = serviceRegistry.getNodes(fullServiceName);
  const healthyNodeNames = serviceRegistry.getHealthyNodes(
    fullServiceName,
    undefined,
    tags,
    undefined,
  ...
  if (tags && tags.length > 0) {
    ...
    const taggedNodes = taggedSets.reduce(
      (acc, set) => new Set([...acc].filter((x) => set.has(x))),
      taggedSets[0] || new Set()
    );
  ...
  const addressToRedirect =
    serviceNode.address +
    (endPoint.length > 0 ? (endPoint[0] == '/' ? endPoint : `/${endPoint}`) : '');
...
const bulkRegisterController = (req, res) => {
  ...
  if (!Array.isArray(services)) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Body must be an array of services' });
    return;
  }
  const results = services.map((service) => {
    try {
      ...
      return serviceResponse
        ? { success: true, service: serviceResponse }
...
const bulkDeregisterController = (req, res) => {
  ...
  if (!Array.isArray(services)) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Body must be an array of {serviceName, nodeName, namespace}' });
    return;
  }
...
const restoreController = (req, res) => {
  ...
  try {
    ...
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Failed to restore registry', error: error.message });
  }
};
...
const setApiSpecController = (req, res) => {
  ...
  try {
    ...
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Invalid API spec: must be valid JSON' });
    return;
  }
  ...
  if (!parsedSpec.openapi && !parsedSpec.swagger) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Invalid API spec: missing openapi or swagger version' });
    return;
  }
  ...
  if (!parsedSpec.info || !parsedSpec.info.title || !parsedSpec.paths) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Invalid API spec: missing required fields (info.title, paths)' });
    return;
  }
  ...
  if (Object.keys(parsedSpec.paths).length === 0) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Invalid API spec: paths cannot be empty' });
    return;
  }
  const fullServiceName = namespace ? `${namespace}:${serviceName}` : serviceName;
  const service = serviceRegistry.registry.get(fullServiceName);
...
const pushHealthController = (req, res) => {
  ...
  if (!serviceName || !nodeName || !status) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Missing serviceName, nodeName, or status' });
    return;
  }
  if (!['healthy', 'unhealthy'].includes(status)) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: "Status must be 'healthy' or 'unhealthy'" });
    return;
  }
  const fullServiceName = namespace ? `${namespace}:${serviceName}` : serviceName;
  const service = serviceRegistry.registry.get(fullServiceName);
...
const updateMetadataController = (req, res) => {
  ...
  if (!serviceName || !nodeName || !metadata) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Missing serviceName, nodeName, or metadata' });
    return;
  }
  const fullServiceName = namespace ? `${namespace}:${serviceName}` : serviceName;
...
const testServiceController = async (req, res) => {
  ...
  if (!service || !service.nodes[nodeName]) {
    return res
      ...
      .json({ message: 'Service or node not found' });
  }
...
const getServiceUptimeController = (req, res) => {
  ...
  res
    .status(statusAndMsgs.STATUS_SUCCESS)
    .json({ serviceName, uptimeMs: uptime, uptimeHuman: `${Math.floor(uptime / 1000)}s` });
};
...
const setCanaryController = (req, res) => {
  ...
  if (!serviceName || percentage === undefined || !Array.isArray(canaryNodes)) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Missing serviceName, percentage, or canaryNodes array' });
    return;
  }
...
const setBlueGreenController = (req, res) => {
  ...
  if (!serviceName || !Array.isArray(blueNodes) || !Array.isArray(greenNodes)) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Missing serviceName, blueNodes, or greenNodes arrays' });
    return;
  }
...
const setIntentionController = (req, res) => {
  ...
  if (!source || !destination || !action) {
    res
      .status(statusAndMsgs.STATUS_GENERIC_ERROR)
      .json({ message: 'Missing source, destination, or action' });
    return;
  }
