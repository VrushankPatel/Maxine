...
class SpiffeSpireController {
  ...
  async rotateCertificates() {
    for (const [nodeId, workload] of this.workloadIdentities) {
      if (workload.expiresAt && workload.expiresAt < new Date(Date.now() + 60 * 60 * 1000)) {
  ...
  async validateCertificate(nodeId, clientCertificate) {
    ...
    try {
      ...
      const clientFingerprint = crypto.createHash('sha256').update(clientCertificate).digest('hex');
  ...
  async authorizeRequest(serviceName, clientSpiffeId, requestedService) {
    ...
    return {
      authorized: false,
      reason: `Access denied: ${clientSpiffeId} not authorized for ${requestedService}`,
    };
  }
  ...
  createMtlsMiddleware() {
    return async (req, res, next) => {
      ...
      if (!validation.valid) {
        return res
          .status(401)
          .json({ error: 'Certificate validation failed: ' + validation.reason });
      }
