...
const perServiceLimiter = rateLimit
  ? rateLimit({
      ...
      keyGenerator: (req) =>
        `${req.query.serviceName || 'unknown'}:${rateLimit ? rateLimit.ipKeyGenerator(req) : req.ip}`,
...
const hasMetrics =
  config.metricsEnabled && !isUltraFastMode && !isExtremeFastMode && !isLightningMode;
...
const circuitOpenBuffer = Buffer.from(
...
const buildMultipleResponseBuffer = (addresses, nodeNames) => {
  ...
  const buf = Buffer.allocUnsafe(totalLen);
  let offset = 0;
  buf.write('{"addresses":[', offset);
  offset += 14;
  for (let i = 0; i < addresses.length; i++) {
    buf.write('"', offset);
    offset += 1;
    buf.write(addresses[i], offset);
    offset += addresses[i].length;
    buf.write(i < addresses.length - 1 ? '",' : '"', offset);
    offset += 1;
  }
  buf.write('],"nodeNames":[', offset);
  offset += 14;
  for (let i = 0; i < nodeNames.length; i++) {
    buf.write('"', offset);
    offset += 1;
    buf.write(nodeNames[i], offset);
    offset += nodeNames[i].length;
    buf.write(i < nodeNames.length - 1 ? '",' : '"', offset);
    offset += 1;
  }
  buf.write('],"healthy":[', offset);
  offset += 12;
  for (let i = 0; i < addresses.length; i++) {
...
const ultraFastDiscovery = async (req, res) => {
  ...
  if (count > 1) {
    ...
    const responseBuffer = buildMultipleResponseBuffer(
...
const normalDiscovery = async (req, res) => {
  ...
  let serviceNodes;
  if (count > 1) {
    serviceNodes = serviceRegistry
      .getHealthyNodes(fullServiceName, null, null, null, null, advancedFilters)
      .slice(0, count);
    ...
    const serviceNode = await discoveryService.getNode(
      fullServiceName,
      ip,
      null,
      null,
  ...
  if (sourceService) {
    ...
    if (
      acl.deny.includes(sourceService) ||
      (acl.allow.length > 0 && !acl.allow.includes(sourceService))
    ) {
      if (hasMetrics && !isHighPerformanceMode && !isUltraFastMode) {
        const latency = Date.now() - startTime;
        metricsService.recordRequest(serviceName, false, latency);
  ...
  if (
    count > 1 ||
    req.query.proxy === 'false' ||
    (req.query.proxy === undefined && !config.defaultProxyMode)
  ) {
    if (hasMetrics && !isHighPerformanceMode && !isUltraFastMode) {
      const latency = Date.now() - startTime;
  ...
  if (
    ...
    !isUltraFastMode &&
    serviceRegistry.isCircuitOpen(fullServiceName, serviceNode.nodeName)
  ) {
    if (hasMetrics && !isHighPerformanceMode && !isUltraFastMode) {
      const latency = Date.now() - startTime;
      metricsService.recordRequest(serviceName, false, latency);
...
const lightningDiscovery = async (req, res) => {
  ...
  let serviceNode = await serviceRegistry.ultraFastGetRandomNode(
    fullServiceName,
    strategy,
    clientId
  );
