...
const databaseDiscoveryController = (req, res) => {
  ...
  const fullServiceName = buildServiceNameCached(
    namespace,
    'default',
    'default',
    serviceName,
  ...
  const databaseNodes = Object.values(nodes).filter(
    (node) => node.metadata && node.metadata.type === 'database'
  );

  if (databaseNodes.length === 0) {
    res
      .status(statusAndMsgs.SERVICE_UNAVAILABLE)
      .json({ message: 'No database nodes found for this service' });
    return;
  }
