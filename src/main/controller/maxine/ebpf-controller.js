...
class EBPFIntegration {
    ...
    getNetworkTopology() {
        ...
        for (const [serviceName, traces] of this.traceBuffers) {
            for (const trace of traces.slice(-50)) { // Last 50 traces
                // Find source and destination nodes
                const sourceNode = Array.from(serviceNodes.values())
                    .find(node => node.address.includes(trace.sourceIP.split('.')[3]));
                const destNode = Array.from(serviceNodes.values())
                    .find(node => node.address.includes(trace.destIP.split('.')[3]));
