...
const predictFaultInjection = (serviceName) => {
    ...
    const failureProbability = Math.min(1, Math.max(0,
        (errorRate * 2) + (avgLatency > 1000 ? 0.3 : 0) + (nodeCount < 2 ? 0.2 : 0)
    ));
    ...
    return {
        ...
        avgLatency,
        nodeCount,
        recommendedAction: failureProbability > 0.7 ? 'inject_failure' :
                          failureProbability > 0.4 ? 'inject_latency' : 'monitor'
    };
};
