...
class DiscoveryService{
    ...
    cache = new LRU({ max: config.highPerformanceMode ? Math.min(config.discoveryCacheMax, 100000) : config.discoveryCacheMax, ttl: config.discoveryCacheTTL });
    serviceKeys = new Map(); // Map serviceName to set of cache keys
    cacheHits = 0;
    cacheMisses = 0;
    aliasCache = new LRU({ max: config.highPerformanceMode ? Math.min(config.aliasCacheMax, 100000) : Math.max(config.aliasCacheMax, 10000), ttl: 900000 }); // Cache for alias resolutions, 15 min TTL, at least 10k
    cacheKeyCache = new LRU({ max: config.highPerformanceMode ? 100000 : 100000, ttl: 900000 }); // Cache for cache key building
