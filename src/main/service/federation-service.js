...
class FederationService {
  ...
  async replicateRegistration(serviceName, serviceData) {
    ...
    const promises = Array.from(this.federatedRegistries.entries()).map(
      async ([name, registry]) => {
  ...
  async replicateDeregistration(serviceName, nodeName) {
    ...
    const promises = Array.from(this.federatedRegistries.entries()).map(
      async ([name, registry]) => {
        ...
        try {
          await axios.delete(
            `${registry.url}/deregister?serviceName=${serviceName}&nodeName=${nodeName}`,
            {
              timeout: config.federationTimeout,
            }
          );
  ...
  performFailoverIfNeeded() {
    ...
    if (
      !currentPrimaryData ||
      !currentPrimaryData.isHealthy ||
      currentPrimaryData.replicationLag > this.replicationLagThreshold
    ) {
