...
class EcsService {
  ...
  async updateServices() {
    ...
    for (const clusterArn of clusters.clusterArns) {
      ...
      for (const serviceArn of services.serviceArns) {
        const serviceDesc = await this.ecs
          .describeServices({ cluster: clusterArn, services: [serviceArn] })
          .promise();
  ...
  async registerEcsService(ecsService, clusterArn) {
    ...
    const tasks = await this.ecs
      .listTasks({ cluster: clusterArn, serviceName: ecsService.serviceName })
      .promise();
    if (tasks.taskArns.length > 0) {
      const taskDesc = await this.ecs
        .describeTasks({ cluster: clusterArn, tasks: tasks.taskArns })
        .promise();
