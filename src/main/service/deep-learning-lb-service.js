...
class DeepLearningLBService {
  ...
  predictNodePerformance(nodeName, timestamp) {
    ...
    const predictedErrorRates = this.predictTimeSeries(
      successRates.map((s) => 1 - s),
      5
    );
    const predictedErrorRate = Math.max(
      0,
      Math.min(1, predictedErrorRates[predictedErrorRates.length - 1])
    );
  ...
  calculateDataQuality(data) {
    ...
    const outliers = data.filter(
      (d) => d.responseTime < lowerBound || d.responseTime > upperBound
    ).length;
    ...
    try {
      ...
      for (const node of nodes) {
        const features = this.extractFeatures(
          node.nodeName,
          this.nodeTimeSeriesData.get(node.nodeName) || []
        );
        if (!features) {
  ...
  async getModelMetrics(serviceName) {
    ...
    try {
      ...
      const normalizedValFeatures = valFeatures.map((f) =>
        this.normalizeFeatures(f, modelData.scaler)
      );
