...
class RegistryService{
    ...
    registerService = (serviceObj) => {
        ...
        if (!service) return;
        const {serviceName, version, namespace, region, zone, datacenter, tenantId, nodeName, address, timeOut, weight, metadata, aliases, apiSpec} = service;
        ...
        if (config.approvalRequired && !config.ultraFastMode) {
            ...
            sRegistry.pendingServices.set(pendingKey, { ...serviceObj, fullServiceName, registeredAt: Date.now() });
        ...
        if (currentNodesCount + effectiveWeight > config.maxInstancesPerService) {
            consoleLog(`Service ${fullServiceName} has reached max instances limit (${config.maxInstancesPerService})`);
        ...
        [...Array(effectiveWeight).keys()].forEach(index => {
            ...
            const nodeObj = {
                ...
                "metadata" : (config.ultraFastMode || config.lightningMode) ? { weight: effectiveWeight } : metadata, // In lightning, only weight for load balancing
    ...
    deregisterService = (serviceName, nodeName, namespace = "default", region = "default", zone = "default", tenantId = "default") => {
        ...
        let toRemove;
        if (config.lightningMode) {
            toRemove = Array.from(service.nodes.values()).filter(n => n.parentNode === nodeName).map(n => n.nodeName);
        } else {
            toRemove = Object.keys(service.nodes).filter(key => service.nodes[key].parentNode === nodeName);
        }
