...
class HealthService {
    ...
    sendAlert(message) {
        if (config.alertWebhook) {
            ...
            axios.post(config.alertWebhook, { message, timestamp: new Date().toISOString() }, { timeout: 5000 }).catch(err => {
    ...
    async checkGrpcHealth(host, port, service = '') {
        return new Promise((resolve) => {
            try {
                ...
                const packageDefinition = protoLoader.loadSync(__dirname + '/../../api-specs/health.proto', {
                    keepCase: true,
                    longs: String,
                    enums: String,
                    defaults: true,
                    ...
                    let isHealthy = false;
                     if (healthType === 'tcp') {
                          ...
                          isHealthy = await this.checkGrpcHealth(host, port, service);
                           ...
                           // HTTP health check
                           isHealthy = await this.checkHttpHealth(node.address, node.metadata.healthEndpoint || '/health', node.metadata.healthMethod || 'GET', node.metadata.healthHeaders || {});
                       }
                      if (isHealthy) {
