...
const serviceNameCache = new LRUCache({
  max: config.highPerformanceMode ? 1000000 : 200000,
  ttl: 900000,
}); // 15 min TTL, optimized for memory
...
const buildServiceNameCached = (tenantId, namespace, region, zone, serviceName, version) => {
  ...
  const fullServiceName =
    region !== 'default' || zone !== 'default'
      ? version
        ? `${tenantPrefix}${namespace}:${region}:${zone}:${serviceName}:${version}`
        : `${tenantPrefix}${namespace}:${region}:${zone}:${serviceName}`
      : version
        ? `${tenantPrefix}${namespace}:${serviceName}:${version}`
