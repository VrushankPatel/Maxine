...
execSync(
  `openssl req -x509 -new -nodes -key ${certsDir}/ca.key -sha256 -days 365 -out ${certsDir}/ca.crt -subj "/C=US/ST=State/L=City/O=Organization/CN=MaxineCA"`
);
...
execSync(
  `openssl req -new -key ${certsDir}/server.key -out ${certsDir}/server.csr -subj "/C=US/ST=State/L=City/O=Organization/CN=localhost"`
);

// Sign server certificate with CA
console.log('Signing server certificate...');
execSync(
  `openssl x509 -req -in ${certsDir}/server.csr -CA ${certsDir}/ca.crt -CAkey ${certsDir}/ca.key -CAcreateserial -out ${certsDir}/server.crt -days 365 -sha256`
);
...
execSync(
  `openssl req -new -key ${certsDir}/client.key -out ${certsDir}/client.csr -subj "/C=US/ST=State/L=City/O=Organization/CN=MaxineClient"`
);

// Sign client certificate with CA
console.log('Signing client certificate...');
execSync(
  `openssl x509 -req -in ${certsDir}/client.csr -CA ${certsDir}/ca.crt -CAkey ${certsDir}/ca.key -CAcreateserial -out ${certsDir}/client.crt -days 365 -sha256`
);
