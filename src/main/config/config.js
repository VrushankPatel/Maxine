...
let lightningDefault = false; // Disable lightning mode, use ultra-fast for maximum performance
let ultraFastDefault = process.env.ULTRA_FAST_MODE !== 'false'; // Ultra-fast mode: extreme performance with minimal features, enabled by default
...
const config = {
  ...
  actuatorEnabled:
    process.env.ACTUATOR_ENABLED === 'true' ||
    (!highPerfDefault &&
      !ultraFastDefault &&
      !extremeFastDefault &&
      process.env.ACTUATOR_ENABLED !== 'false') ||
    isTestMode,
  dashboardEnabled:
    process.env.DASHBOARD_ENABLED === 'true' ||
    (!extremeFastDefault && process.env.DASHBOARD_ENABLED !== 'false'),
  statusMonitorEnabled: process.env.STATUS_MONITOR_ENABLED === 'true' || false,
  serverSelectionStrategy: process.env.SERVER_SELECTION_STRATEGY
    ? constants.SSS[process.env.SERVER_SELECTION_STRATEGY] || constants.SSS.RR
    : constants.SSS.RR,
  logFormat:
    process.env.LOG_FORMAT === 'PLAIN' ? constants.LOG_FORMATS.PLAIN : constants.LOG_FORMATS.JSON,
  discoveryCacheTTL: process.env.DISCOVERY_CACHE_TTL
    ? parseInt(process.env.DISCOVERY_CACHE_TTL)
    : ultraFastDefault
      ? 7200000
      : lightningDefault
        ? 30000
        : 3600000, // 30s in lightning for faster cache
  discoveryCacheMax: process.env.DISCOVERY_CACHE_MAX
    ? parseInt(process.env.DISCOVERY_CACHE_MAX)
    : ultraFastDefault
      ? 2000000
      : highPerfDefault
        ? 2000000
        : lightningDefault
          ? 1000000
          : 500000,
  aliasCacheMax: process.env.ALIAS_CACHE_MAX
    ? parseInt(process.env.ALIAS_CACHE_MAX)
    : ultraFastDefault
      ? 1000000
      : highPerfDefault
        ? 1000000
  ...
  clusteringEnabled:
    process.env.CLUSTERING_ENABLED !== 'false' &&
    !process.argv.some((arg) => arg.includes('mocha')), // Enabled in lightning mode for better performance, note: registry is per-worker, use Redis for shared
  numWorkers: process.env.NUM_WORKERS
    ? parseInt(process.env.NUM_WORKERS)
    : require('os').cpus().length,
  healthCheckEnabled:
    (process.env.HEALTH_CHECK_ENABLED === 'true' && !ultraFastDefault && !extremeFastDefault) ||
    lightningDefault, // Enable health checks in lightning mode for all features
  ...
  dependencyCallThreshold: process.env.DEPENDENCY_CALL_THRESHOLD
    ? parseInt(process.env.DEPENDENCY_CALL_THRESHOLD)
    : 10,
  dependencyMaxAge: process.env.DEPENDENCY_MAX_AGE
    ? parseInt(process.env.DEPENDENCY_MAX_AGE)
    : 24 * 60 * 60 * 1000, // 24 hours
  dependencyAnalysisInterval: process.env.DEPENDENCY_ANALYSIS_INTERVAL
    ? parseInt(process.env.DEPENDENCY_ANALYSIS_INTERVAL)
    : 300000, // 5 minutes
  ...
  metricsEnabled:
    (process.env.METRICS_ENABLED !== 'false' && !ultraFastDefault && !extremeFastDefault) ||
  ...
  healthCheckInterval: process.env.HEALTH_CHECK_INTERVAL
    ...
    : ultraFastDefault
      ? 30000
      : 30000, // Same interval for reliability
  healthCheckConcurrency: process.env.HEALTH_CHECK_CONCURRENCY
    ? parseInt(process.env.HEALTH_CHECK_CONCURRENCY)
    : ultraFastDefault
      ? 100
      : 50, // Higher concurrency in ultra-fast for speed
  ...
  circuitBreakerEnabled:
    (process.env.CIRCUIT_BREAKER_ENABLED !== 'false' && !highPerfDefault && !ultraFastDefault) ||
    lightningDefault,
  circuitBreakerFailureThreshold: process.env.CIRCUIT_BREAKER_FAILURE_THRESHOLD
  ...
  http2Enabled: process.env.HTTP2_ENABLED === 'true' && !ultraFastDefault && !isTestMode, // Disabled in ultra-fast mode for maximum performance, enabled only when explicitly requested
  ...
  federationTimeout: process.env.FEDERATION_TIMEOUT
    ? parseInt(process.env.FEDERATION_TIMEOUT)
    : 5000,
  federationRetryAttempts: process.env.FEDERATION_RETRY_ATTEMPTS
    ? parseInt(process.env.FEDERATION_RETRY_ATTEMPTS)
    : 3,
