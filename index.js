...
if (config.ultraFastMode) {
  ...
  const {
    LightningServiceRegistrySimple,
  ...
  const __stringifyRegister = __stringify(registerResponseSchema);
  ...
  const __stringifyDiscover = __stringify(discoverResponseSchema);
  ...
  const __stringifySuccess = __stringify(successResponseSchema);
  ...
  const __stringifyServers = __stringify(serversResponseSchema);
  ...
  // const __stringifyMetrics = __stringify(metricsResponseSchema);
  ...
  const handleDiscover = (req, res, _query, _body) => {
    try {
      ...
      const distribution = serviceRegistry.getTrafficDistribution
    ...
    if (metadata) {
      if (typeof metadata !== 'object') {
        ...
        if (
          ...
          (typeof metadata.weight !== 'number' || metadata.weight < 1 || metadata.weight > 10)
        ) {
        ...
        if (metadata.healthCheck) {
          ...
          if (typeof hc !== 'object') {
            ...
            if (
              ...
              (typeof hc.interval !== 'number' || hc.interval < 1000)
            ) {
  ...
  const handleRegister = (req, res, _query, _body) => {
    try {
      ...
      if (validationErrors.length > 0) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.__stringify({ error: 'Validation failed', details: validationErrors }));
        return;
      }
  ...
  if (config.samlEnabled && config.samlEntryPoint && config.samlIssuer && config.samlCert) {
    ...
    routes.set('POST /auth/saml/callback', (req, res, _query, _body) => {
      passport.authenticate('saml', (err, user, _info) => {
        ...
        // Create JWT for user
        const userToken = jwt.sign(
          { id: user.nameID, email: user.email, name: user.displayName },
          config.jwtSecret,
          { expiresIn: config.jwtExpiresIn }
        );
        res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  const handleIstioConfig = (req, res, _query, _body) => {
    try {
      ...
      for (const [serviceName, _nodes] of services) {
        ...
        const errorRate = serviceRegistry.getErrorRate
          ? serviceRegistry.getErrorRate(serviceName)
          : 0.05; // Default 5%
        const avgResponseTime = serviceRegistry.getAvgResponseTime
          ? serviceRegistry.getAvgResponseTime(serviceName)
          : 100; // Default 100ms
        const trafficVolume = serviceRegistry.getTrafficVolume
          ? serviceRegistry.getTrafficVolume(serviceName)
          : 1000; // Default requests/min
...
<body>
    ...
    <ul>
        ${eventHistory
          .slice(-10)
          .map((e) => `<li>${new Date(e.timestamp).toISOString()}: ${e.event}</li>`)
          .join('')}
    </ul>
</body>
</html>`;
...
<body>
    ...
    <script>
        const analytics = ${JSON.__stringify(analytics)};
  ...
  routes.set('GET /dependency-graph', (req, res, _query, _body) => {
    try {
      ...
      const graphJson = JSON.__stringify(graph);
      const cyclesJson = JSON.__stringify(cycles);
      const callLogsJson = JSON.__stringify(callLogs);
      const html = `
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <script>
        ...
        function exportJSON() {
            const dataStr = JSON.__stringify(graphData, null, 2);
  ...
  const loadEventHistory = () => {
    if (
      ...
      config.persistenceType === 'file' &&
      fs.existsSync(eventHistoryFile)
    ) {
      try {
  ...
  const broadcast = (event, data) => {
    ...
    const message = JSON.__stringify(messageObj);
    // Store in history
  ...
  const matchesFilter = (event, data, filter) => {
    ...
    if (filter.serviceName && data.serviceName && filter.serviceName !== data.serviceName)
      return false;
      ...
      ws.on('message', (message) => {
        try {
          ...
          if (data.auth) {
            ...
            ws.send(JSON.__stringify({ type: 'auth_required' }));
            ws.close();
            return;
          }
          if (data.subscribe) {
            ...
            if (data.subscribe.event === 'admin_event' && (!user || user.role !== 'admin')) {
              ws.send(JSON.__stringify({ type: 'error', message: 'Insufficient permissions' }));
              return;
            }
            clientFilters.set(ws, data.subscribe);
            ws.send(JSON.__stringify({ type: 'subscribed', filter: data.subscribe }));
          } else if (data.unsubscribe) {
            clientFilters.set(ws, null);
            ws.send(JSON.__stringify({ type: 'unsubscribed' }));
          } else if (data.refresh_token) {
            const user = clientAuth.get(ws);
            if (!user) {
              ws.send(JSON.__stringify({ type: 'error', message: 'Not authenticated' }));
              return;
            }
            try {
              const newToken = jwt.sign(
